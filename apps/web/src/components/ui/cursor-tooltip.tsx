...
function TooltipContent({ tooltip }: { tooltip: { text: string; x: number; y: number } }) {
  ...
  return (
    <div
      ref={ref}
      className="pointer-events-none fixed left-0 top-0 z-[150] max-w-xs rounded-md bg-foreground px-2 py-1.5 text-xs font-medium text-background whitespace-pre-line"
    >
      {tooltip.text}
    </div>
  );
