 ...
 * events (the pairing history sync and the live socket). The protocol pushes
 * state instead of answering queries, so this store is the only thing the
 * read tools have — anything not ingested here is unreachable later.
 *
 * Inputs are structural subsets of Baileys' event payloads, navigated
 * defensively (SDK output is untrusted input): rows are only written when a
 * jid and a usable text resolve. Media never lands here — a message whose
 * content is media is kept as a bracketed marker ("[image] caption"), and
 * unrenderable protocol events (reactions, key distribution) are skipped.
 */
...
export interface WaContactInput {
  ...
  name?: string | null;
  notify?: string | null;
  /** The contact's phone-number jid (…@s.whatsapp.net), when the id is a LID. */
...
/** Chats that are not conversations: WhatsApp statuses, channels, broadcast lists. */
function isIgnoredJid(jid: string): boolean {
  return jid.endsWith("@broadcast") || jid.endsWith("@newsletter");
}

/** The digits of a phone-number jid ("4917…@s.whatsapp.net"), or "" for LIDs/groups. */
...
/**
 * A message's agent-facing text: real text as-is, media as a bracketed
 * marker (plus caption/filename where one exists), null for protocol events
 * that render as nothing (reactions, edits, key distribution) — callers skip
 * those rows entirely.
 */
...
function writeMessages(messages: WaMessageInput[], updatedAt: string): void {
  ...
  for (const message of messages) {
    ...
    // coverage for people who aren't in the address book.
...
/** One "messaging-history.set" payload — a pairing sync chunk. */
...
/** Live "messages.upsert" batches, and the echo of a message the agent sent. */
...
/** Recent chats, newest first, optionally filtered by name/number. */
...
/** One chat's messages in chronological order, newest window first when capped. */
...
/** Contacts matching a name, push name or number fragment. */
...
/** Chats whose resolved display name matches — the tools' name → chat resolution. */
...
/** A single chat's display name; falls back to the jid's number for unknown chats. */
