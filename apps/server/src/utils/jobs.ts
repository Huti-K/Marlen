 ...
 * (automations, the learn loops, delegate workers) composes these instead of
 * hand-rolling its own timers, guards and pools. Deliberately no persistence
 * and no generic retry: all recovery state lives in SQLite (cursors, run
 * rows), so a restart resumes from data, not from a queue.
 */
...
export interface NightlyJobOptions {
  ...
  name: string;
  /** Fixed, server-defined cron expression (user-configurable schedules are
   *  automations/scheduler.ts's concern, not this class's). */
  cron: string;
 ...
 * (reason "boot") so work that came due while the process was down isn't
 * stuck waiting for the next cron slot, then runs on the cron (reason
 * "scheduled") in the given IANA timezone, or the server's local time when
 * none is passed. A failed run is logged without killing the schedule;
 * stop() destroys the cron task while a run already executing finishes on
 * its own. node-cron bakes the timezone into a task at creation, so a
 * timezone change reaches a started job only via reschedule().
 */
export class NightlyJob {
  ...
  /** Rebuild the cron task against a new timezone. Only the schedule moves —
   *  no catch-up run fires, and a run already executing finishes on its own.
   *  A no-op while stopped: only start() arms a stopped job. */
