 ...
 * generation counter. The invariants every consumer relies on:
 *
 * - Concurrent callers share one in-flight fetch per key instead of each
 *   firing their own.
 * - `invalidate` bumps the key's generation. A fetch dispatched under an
 *   older generation never repopulates the cache when it resolves, and a new
 *   caller never joins it — it starts a fresh fetch instead. Otherwise a
 *   fetch started before a mutation could re-cache the pre-mutation value for
 *   a full TTL after the mutation already invalidated.
 * - A settled fetch clears only its own in-flight slot — a fresh fetch
 *   dispatched after it (because it was already stale) may have replaced it.
 * - Failed fetches never populate the cache; the rejection propagates to
 *   every caller sharing the fetch.
 * - Entries are never evicted on a timer. Staleness is a timestamp
 *   comparison against `ttlMs` at read time, so a stale value stays
 *   peekable indefinitely for stale-while-revalidate readers.
 */

export interface FetchOutcome<T> {
  value: T;
  /**
   * Whether the value was written to the cache — false when an invalidate
   * landed after the fetch was dispatched, in which case the value reflects
   * pre-invalidation state and must not be treated as current.
   */
  stored: boolean;
}
...
export function createFetchCache<T>(opts: { ttlMs: number }): FetchCache<T> {
  ...
  return {
    ...
    fetch(key, fetcher) {
      ...
      // one left over from before an invalidate call is stale, so start a
      // fresh fetch instead of joining it.
      ...
      const promise: Promise<FetchOutcome<T>> = fetcher()
        ...
        .finally(() => {
          // Only clear our own slot — a fresh fetch dispatched after us may
          // have replaced it.
